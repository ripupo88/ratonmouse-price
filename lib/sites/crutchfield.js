...
class Crutchfield {
  ...
  findPriceOnPage($) {
    ...
    if (!priceString) {
      logger.error(
        "price was not found on crutchfield page, uri: %s",
        this._uri
      );
      return -1;
    }
  ...
  findCategoryOnPage($) {
    const categoryString = $(
      "#breadCrumbNav .breadcrumb-item:nth-child(2)"
    ).text();
    if (!categoryString || categoryString.length < 1) {
      logger.error(
        "category not found on crutchfield page, uri: %s",
        this._uri
      );
      return null;
    }
