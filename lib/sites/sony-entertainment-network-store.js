...
class SonyEntertainmentNetworkStoreSite {
  constructor(uri) {
    ...
    if (!SonyEntertainmentNetworkStoreSite.isSite(uri)) {
      throw new Error(
        "invalid uri for Sony Entertainment Network Store: %s",
        uri
      );
    }
  ...
  findPriceOnPage(pageData) {
    ...
    if (!pageData || !pageData.default_sku) {
      logger.error(
        "price was not found on sony store page, uri: %s",
        this._uri
      );
      return -1;
    }
    ...
    if (
      pageData.default_sku.rewards &&
      pageData.default_sku.rewards.length > 0
    ) {
  ...
  findNameOnPage(pageData) {
    ...
    if (!name || name.length < 1) {
      logger.error(
        "name not found on sony entertainment network store page, uri: %s",
        this._uri
      );
      return null;
    }
  ...
  static isSite(uri) {
    if (
      uri.indexOf("store.sonyentertainmentnetwork.com") > -1 ||
