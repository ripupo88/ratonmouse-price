...
class AmazonSite {
  ...
  findCategoryOnPage($) {
    ...
    if (!amazonCategory || amazonCategory.length < 1) {
      ...
      if (
        !amazonCategory ||
        amazonCategory.length < 1 ||
        amazonCategory === "Back to results"
      ) {
  ...
  findNameOnPage($, category) {
    ...
    if (category === siteUtils.categories.DIGITAL_MUSIC) {
      ...
      name =
        $("#ArtistLinkSection").text().trim() +
        ": " +
        $("#dmusicProductTitle_feature_div").text().trim();
      /* eslint-enable prefer-template */
    } else {
      const selectors = ["#btAsinTitle", "#productTitle", "#title"];
